 ...
 *
 * @todo Update cookie expiry at every authentication
 *
	...
	return async (req, res) => {
		...
		try {
			...
			if (sessionId) {
				...
				res.setHeader('Set-Cookie', clear('session_id'))
			}
		}
...
/**
 * Handles various types of errors, especially the ones that Twitter
 * returns. It returns a request-response handler that sets headers and status
 * codes, and renders an error page.
 *
 * @param  {Object} error
 * @return {Function}
 */
...
function getErrorMessage(error, rateLimit) {
	...
	if (typeof rateLimit === 'object') {
		if (getStatusCode(error) === httpStatus.TOO_MANY_REQUESTS) {
...
function getStatusCode(error) {
	const errorCodeToStatusCodeMap = {
		32: httpStatus.NOT_AUTHORIZED,
		34: httpStatus.NOT_FOUND,
		63: httpStatus.FORBIDDEN,
		64: httpStatus.FORBIDDEN,
		87: httpStatus.FORBIDDEN,
		88: httpStatus.TOO_MANY_REQUESTS,
		89: httpStatus.FORBIDDEN,
		99: httpStatus.FORBIDDEN,
		130: httpStatus.SERVICE_UNAVAILABLE,
		135: httpStatus.NOT_AUTHORIZED,
		215: httpStatus.BAD_REQUEST,
		251: httpStatus.GONE,
		325: httpStatus.BAD_REQUEST,
		415: httpStatus.FORBIDDEN,
		416: httpStatus.NOT_AUTHORIZED,
		417: httpStatus.NOT_AUTHORIZED,
	}
	if (error.errors) {
		return errorCodeToStatusCodeMap[error.errors[0].code] || httpStatus.INTERNAL_SERVER_ERROR
