...
function getRequestToken(req) {
	if (!client) initClient()
	const redirectUrl = getUrl(req) + authentication.TWITTER_REDIRECT_PATH
...
async function getTimeline(offsetId) {
	...
	const tweetsData = await client.get(apiEndpoints.HOME_TIMELINE, params)
...
function verifyCredentials(key, secret) {
	...
	return client.get(apiEndpoints.VERIFY_CREDENTIALS)
}
