...
test('Initialising client passes correct connection params to Redis client', t => {
	...
	const spy = sinon.spy(redis, 'createClient')
	redisInstance.initClient(REDIS_CONNECTION_OPTIONS)
	t.deepLooseEqual(spy.args[0][0], { ...REDIS_CONNECTION_OPTIONS, tls: {} })
	t.end()
})

/**
 * Tests whether the session id is returned when creating a session. The session
 * is valid if it’s a URL safe base64 encoded string of 32 characters plus the
 * AUTO_INCR added at the end.
 *
...
test('Get user session should return user credentials', async t => {
	const client = proxyquire('../lib/redis-instance', {
		redis
	})
	const userCredentials = await client.getUserSession()
 ...
 */
test('Get user session should throw if it returns bad user credentials', async t => {
	const client = proxyquire('../lib/redis-instance', {
		redis: redisNoUserCredentials
	})
	t.throws(await client.getUserSession())
	t.end()
})
